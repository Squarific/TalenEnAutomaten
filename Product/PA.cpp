...
Automaton productAutomaton(Automaton& automaton_1, Automaton& automaton_2) {
	...
	for (int i = 0; i < automaton_1.state_amount; i++) {
		for (int j = 0; j < automaton_2.state_amount; j++) {
			product_automaton.addState(PState(automaton_1.states[i].name + automaton_2.states[j].name));
		}
	}
	...
	// Add end states
	for (int i = 0; i < automaton_1.states.size(); i++) {
		if (automaton_1.states[i].isEnd) {
			for (int j = 0; j < automaton_2.states.size(); j++) {
				if (automaton_2.states[j].isEnd) {
					product_automaton.getState(automaton_1.states[i].name + automaton_2.states[j].name)->setEnd(true);
				}
			}
		}
	}
